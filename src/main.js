...
import { PROF, PICO, PROF_TOOL, TRAITS, NAMES, COLORS, SKINS, HAIRS, HAIR_STYLES, SUPPLIES, BLDGS, CRAFTS, LORE, ENEMY_TYPES, MAP_W, MAP_H, TS, TICK_MS, SEASONS, BCAT_DATA, } from './data/index';
// ── УТИЛИТЫ ──────────────────────────────────────────────────────────────────
...
function playBeep(freq, duration, type = 'sine', volume = 0.07) {
    const ctx = initAudio();
    if (!ctx)
        return;
...
document.addEventListener('pointerdown', () => { const ctx = initAudio(); if (ctx && ctx.state === 'suspended')
...
function buildSetup() {
    ...
    startBtn.addEventListener('click', () => {
        SUPPLIES.forEach(s => { if (supQty[s.id] > 0)
...
function genMap() {
    ...
    const at = (c, r) => c >= 0 && c < MAP_W && r >= 0 && r < MAP_H ? G.tiles[r * MAP_W + c] : null;
    ...
    for (let dr = 0; dr < 5; dr++)
        for (let dc = 0; dc < 6; dc++)
            set(wc + dc, wr + dr, 'water');
    for (let r2 = wr + 5; r2 < wr + 10; r2++)
        set(wc + 2, r2, 'water');
...
function renderMap() {
    ...
    G.tiles.forEach(ti => {
        ...
        ti._el = el;
        refreshTileEl(ti);
        el.addEventListener('click', e => { e.stopPropagation(); if (dragMoved)
            return; onTileClick(ti); });
        g.appendChild(el);
    });
    refreshSprites();
}
...
function refreshTileEl(ti) {
    ...
    if (ti.bldg) {
        ...
        if (ti.bldg.field && ti.bldg.buildTime <= 0) {
            ...
            if (phase === 'seeding')
                ico.textContent = ci.seeding;
            else if (phase === 'growing')
                ico.textContent = gv < 40 ? ci.growing25 : ci.growing75;
            else if (phase === 'harvest')
                ico.textContent = ci.harvest;
            else
                ico.textContent = '🟫';
    ...
    if (ti.resPile && (ti.resPile.amount > 0 || ti.resPile.type === 'supplies')) {
        ...
        if (ti.resPile.type === 'supplies') {
            ...
            const icons = { wood: '🪵', stone: '🪨', metal: '⚙', berries: '🫐', mushrooms: '🍄', food: '🌾', water: '💧', copper: '🟤' };
    ...
    if ((ti.type === 'forest' || ti.type === 'ore' || ti.type === 'rock') && ti.resAmt > 0 && !ti.bldg) {
        ...
        lbl.className = 'tlbl';
        lbl.textContent = String(ti.resAmt);
        el.appendChild(lbl);
    }
    if (ti.bldg && ti.bldg.field && ti.bldg.buildTime <= 0 && ti.bldg.crop) {
        ...
        const phShort = { seeding: 'SEED', growing: 'GRW ' + Math.round(ti.bldg.growth || 0) + '%', harvest: 'RIPE' };
...
window.addEventListener('mousemove', e => { if (drag && dragO) {
...
function showTilePop(ti) {
    ...
    const titles = { grass: 'Grassland', forest: 'Forest', water: 'Water Source', soil: 'Fertile Soil', rock: 'Rocky Ground', ore: 'Ore Deposit', road: 'Old Road' };
    let title = ti.bldg ? (BLDGS.find(b => b.id === ti.bldg.id) || { name: 'HQ' }).name : titles[ti.type] || ti.type;
    let body = '';
    if (ti.bldg) {
        if (ti.bldg.buildTime > 0)
            body = 'Under construction: ' + Math.round((1 - ti.bldg.buildTime / ti.bldg.totalTime) * 100) + '%';
        else {
            if (ti.bldg.id === 'storehouse' || ti.bldg.id === 'hq')
                body = 'Contents: ' + (Object.entries(G.res).filter(([, v]) => v > 0).map(([k, v]) => k + ': ' + Math.floor(v)).join(', ') || 'empty');
            if (ti.bldg.field)
                body = 'Crop: ' + (ti.bldg.crop || 'none') + ' · ' + ((ti.bldg.phase || '?').toUpperCase()) + ' ' + Math.round(ti.bldg.growth || 0) + '%';
            if (ti.bldg.id === 'workshop')
                body = 'Tap CRAFT tab to make tools';
        }
    }
    else {
        if (ti.type === 'forest')
            body = 'Timber available: ' + ti.resAmt;
        else if (ti.type === 'ore')
            body = 'Ore: ' + ti.resAmt;
        else if (ti.type === 'rock')
            body = 'Stone: ' + ti.resAmt;
        else if (ti.type === 'water')
            body = 'Permanent water source\nTap → Fetch Water to send a Porter here.\nWater stock: ' + Math.floor(G.res.water);
        else if (ti.type === 'soil')
            body = 'Fertile soil — good for fields';
        if (ti.res && ti.resAmt > 0)
            body += ' · ' + ti.res + ': ' + ti.resAmt;
    ...
    if (!ti.bldg) {
        if (ti.type === 'water')
            actions.push({ label: '💧 Fetch Water (Porter)', fn: () => assignWaterFetch(ti) });
        if (ti.type === 'forest' && ti.resAmt > 0)
            actions.push({ label: '🪵 Gather Wood', fn: () => assignPriorityTask('LUMBERJACK', ti) });
        if (ti.type === 'ore' && ti.resAmt > 0)
            actions.push({ label: '⛏ Mine Ore', fn: () => assignPriorityTask('MINER', ti) });
        if (ti.type === 'rock' && ti.resAmt > 0)
            actions.push({ label: '🪨 Quarry Stone', fn: () => assignPriorityTask('STONEMASON', ti) });
        if ((ti.res === 'berries' || ti.res === 'mushrooms') && ti.resAmt > 0)
            actions.push({ label: '🧺 Gather', fn: () => assignPriorityTask('GATHERER', ti) });
        if (ti.resPile && ti.resPile.amount > 0)
            actions.push({ label: '🎒 Carry to Store', fn: () => assignPriorityTask('PORTER', ti) });
    }
    if (ti.bldg?.id === 'workshop' && ti.bldg.buildTime <= 0)
        actions.push({ label: '🔧 Open Workshop', fn: () => openWorkshop() });
    if (ti.bldg?.id === 'forge' && ti.bldg.buildTime <= 0)
        actions.push({ label: '⚒ Open Forge', fn: () => openCraftShop('forge') });
    if (ti.bldg?.id === 'well' && ti.bldg.buildTime <= 0)
        actions.push({ label: '💧 Fetch Water (Porter)', fn: () => assignWaterFetch(ti) });
...
function placeHQ(ti) {
    ...
    ti.bldg = { id: 'hq', buildTime: 0, name: 'HQ', ico: '🏚', lv: 1, cat: 'SHELTER' };
    G.hqCol = ti.col;
    G.hqRow = ti.row;
    G.hqPlaced = true;
    G.buildings.push({ id: 'hq', col: ti.col, row: ti.row, lv: 1 });
...
function finishPlace(bd, ti, crop) {
    if (crop)
        G.res.seeds = Math.max(0, G.res.seeds - 5);
    ti.bldg = { id: bd.id, buildTime: bd.time, totalTime: bd.time, lv: 1, field: !!bd.field, crop, growth: 0, phase: 'seeding', seedTimer: 0, paused: false, name: bd.name, ico: bd.ico, cat: bd.cat };
    G.buildings.push({ id: bd.id, col: ti.col, row: ti.row, lv: 1, paused: false, field: !!bd.field, crop, growth: 0, phase: 'seeding', seedTimer: 0 });
    if (bd.shelter)
        G.shelter += bd.shelter;
...
function dollHTML(c) {
    if (!c.visual)
        c.visual = { skin: pick(SKINS), hair: pick(HAIRS), hairStyle: pick(HAIR_STYLES), body: c.color };
    const v = c.visual;
...
function selCol(id) {
    ...
    renderSidebar();
    if (G.selectedCol !== null)
        showCdet(G.selectedCol);
    else
...
function showCdet(id) {
    ...
    document.getElementById('cdname').style.color = c.color;
    ...
    document.querySelectorAll('.cds').forEach(s => s.addEventListener('click', () => {
        ...
        if (col) {
            col.prefs[el.dataset.p] = parseInt(el.dataset.s);
            showCdet(id);
        }
    }));
...
function renderBuild() {
    ...
    CATS.forEach(cat => {
        ...
        b.title = cat;
        b.setAttribute('aria-label', cat);
        b.textContent = BCAT_DATA[cat];
    ...
    if (activeCat === 'OTHER' && !G.hqPlaced) {
        ...
        hqBtn.addEventListener('click', () => {
            G.placingBldg = { isHQ: true, ico: '🏚', name: 'HEADQUARTERS' };
    ...
    BLDGS.filter(b => b.cat === activeCat).forEach(bd => {
        ...
        if (!lk)
            btn.addEventListener('click', () => showBuildConfirm(bd));
...
function resourceIcon(k) {
    return ({ food: '🌾', water: '💧', cooked: '🍞', wood: '🪵', stone: '🪨', metal: '⚙️', copper: '🟤', meds: '💊', seeds: '🌱', cloth: '🧶' }[k] || '⬡');
}
function canAffordCost(cost) {
    return Object.entries(cost).every(([r, a]) => (G.res[r] || 0) >= a);
}
function showBuildConfirm(bd) {
    if (bd.isHQ) {
        if (G.hqPlaced) {
            addLog('HQ already built!', 'warn');
            return;
        }
        G.placingBldg = bd;
    ...
    if (!G.hqPlaced) {
        addLog('Place your Headquarters first! (BUILD → OTHER)', 'danger');
        return;
    }
    const affordable = canAffordCost(bd.cost);
    G.paused = true;
    updPauseBtn();
    document.getElementById('mdt').textContent = bd.ico + ' ' + bd.name;
    document.getElementById('mdt').style.color = affordable ? 'var(--accent2)' : 'var(--danger)';
    const costRows = Object.entries(bd.cost).map(([r, a]) => {
        const have = Math.floor(G.res[r] || 0);
        const miss = have < a;
    ...
    if (buildBtn.disabled) {
        buildBtn.style.opacity = '.45';
        buildBtn.style.cursor = 'not-allowed';
    }
    buildBtn.addEventListener('click', () => {
        if (!canAffordCost(bd.cost)) {
            addLog('Not enough resources for ' + bd.name, 'warn');
            return;
        }
        if (bd.field && G.res.seeds <= 0) {
            addLog('No seeds to plant!', 'warn');
            return;
        }
        if (bd.field)
            bd._selectedCrop = selectedCrop;
        G.placingBldg = bd;
    ...
    cancelBtn.className = 'mb2';
    cancelBtn.textContent = 'CANCEL';
    cancelBtn.addEventListener('click', () => { document.getElementById('mdbg').classList.remove('show'); G.paused = false; updPauseBtn(); });
    be.appendChild(buildBtn);
    be.appendChild(cancelBtn);
    document.getElementById('mdbg').classList.add('show');
    window.selectCrop = function (id) {
        selectedCrop = id;
        document.querySelectorAll('[id^="crop-"]').forEach(b => b.style.borderColor = 'var(--border)');
...
function renderAssign() {
    ...
    if (!alive.length) {
        el.innerHTML = '<div style="font-size:9px;color:var(--textdim);padding:4px">No survivors</div>';
        return;
    }
    alive.forEach(col => {
        ...
        nameSpan.className = 'an';
        nameSpan.style.color = col.color;
        ...
        sel.addEventListener('change', e => {
            if (col.tool && col.tool.type && col.tool.type !== '—') {
                returnToolToStock(col.tool);
                col.tool = { type: '—', dur: 0 };
            }
            col.role = e.target.value;
            if (G.enemies?.length && col.role !== 'GUARD') {
                col.targetCol = G.hqCol;
                col.targetRow = G.hqRow;
                col.action = 'FLEEING';
            }
            ...
            if (needed) {
                ...
                if (t) {
                    col.tool = t;
                    addLog(col.name + ' → ' + col.role + ' (' + t.type + ')', 'good');
                }
                else
                    addLog('⚠ ' + col.name + ' → ' + col.role + ': no ' + needed + '!', 'danger');
            }
            else {
                col.tool = { type: '—', dur: 100 };
                addLog(col.name + ' → ' + col.role);
            }
            renderSidebar();
        });
...
function addLog(msg, type = 'normal') {
    ...
    G.log.unshift({ t: 'D' + G.day + ' ' + h + ':' + m, msg, type: type, ico });
    if (G.log.length > 50)
        G.log.pop();
...
function renderLog() {
    document.getElementById('logp').innerHTML = G.log.slice(0, 14).map((e) => `<div class="ll"><span class="lt">${e.t}</span><span class="lm ${e.type}"><span class="li-ico">${e.ico || '·'}</span>${e.msg}</span></div>`).join('');
}
// ── RESOURCES ─────────────────────────────────────────────────────────────────
const lastRes = { food: null, water: null, cooked: null, wood: null, stone: null, metal: null, copper: null, meds: null, seeds: null, cloth: null };
function renderResources() {
    ...
    ['food', 'water', 'cooked', 'wood', 'stone', 'metal', 'copper', 'meds', 'seeds', 'cloth'].forEach(k => {
        ...
        if (lastRes[k] !== null && v !== lastRes[k]) {
            ...
            if (box) {
                box.classList.remove('res-up', 'res-down');
                void box.offsetWidth;
                box.classList.add(v > lastRes[k] ? 'res-up' : 'res-down');
            }
            if (v > lastRes[k])
                up = true;
            else
...
function getTarget(c) {
    ...
    const ftile = (type, needRes = false) => G.tiles.find(t => t.type === type && (!needRes || t.resAmt > 0)) || null;
    if (c.sleeping)
        return hq;
    if (c.priorityTarget) {
...
function doWork(c) {
    ...
    switch (c.role) {
        ...
        case 'MINER': {
            ...
            if (ti && Math.random() < chance * 0.7) {
                const a = 1;
                ti.resAmt -= a;
                const hp2 = G.colonists.some(x => !x.dead && x.role === 'PORTER'), hs2 = G.buildings.some(b => b.id === 'storehouse' || b.id === 'hq');
                if (hp2 && hs2)
                    addPile(ti, 'metal', a);
                else
                    G.res.metal = (G.res.metal || 0) + a;
                refreshTileEl(ti);
                c.action = 'MINING';
            }
            else
                c.action = ti ? 'MINING' : 'WALKING';
            break;
        }
        case 'STONEMASON': {
            ...
            if (ti && Math.random() < chance) {
                const a = Math.min(2, ti.resAmt);
                ti.resAmt -= a;
                const hp3 = G.colonists.some(x => !x.dead && x.role === 'PORTER'), hs3 = G.buildings.some(b => b.id === 'storehouse' || b.id === 'hq');
                if (hp3 && hs3)
                    addPile(ti, 'stone', a);
                else
                    G.res.stone = (G.res.stone || 0) + a;
                refreshTileEl(ti);
                c.action = 'QUARRYING';
            }
            else
                c.action = ti ? 'QUARRYING' : 'WALKING';
            break;
        }
        case 'FARMER': {
            ...
            if (!c.carryAmt) {
                ...
                if (piles.length > 0) {
                    ...
                    if (dist < 0.8) {
                        ...
                        c.action = 'FETCHING ' + pile.resPile.type;
                }
                else
                    c.action = 'IDLE';
            }
    ...
    if (c.role && c.action !== 'IDLE')
        c.skill[c.role] = Math.min(100, (c.skill[c.role] || 0) + (Math.random() < 0.04 ? 1 : 0));
    ...
    if (hour === 8 || hour === 19) {
        ...
        alive.forEach(c => {
            if (hasMeal) {
                ...
                c.hunger += 20;
                addLog(c.name + ' went hungry — no food!', 'danger');
            }
            if (hasWater) {
                G.res.water -= 0.3;
                c.thirst = Math.max(0, c.thirst - 35);
            }
            else {
                c.thirst += 25;
                addLog(c.name + ' went thirsty — no water!', 'danger');
            }
        });
    }
...
function addNewCol() {
    ...
    const maxHp = Math.round(100 * tr.hp);
    const newCol = {
        id: G.colonists.length, name, gender: (Math.random() > 0.5 ? 'M' : 'F'),
        color: COLORS[G.colonists.length % COLORS.length],
        trait: tr.id, prefs: prefs, role: PROF[0], skill: {}, combatSkill: 0,
...
function tickConstruction() {
    G.tiles.filter(t => t.bldg && t.bldg.buildTime > 0).forEach(ti => {
        ...
        if (ti.bldg.buildTime <= 0) {
            addLog(ti.bldg.ico + ' ' + ti.bldg.name + ' complete!', 'good');
            refreshTileEl(ti);
        }
        else
            refreshTileEl(ti);
...
function renderAdvisor() {
    ...
    const tips = [];
    if (!G.hqPlaced)
        tips.push('📍 BUILD → OTHER to place HQ');
    if (G.hqPlaced && !G.buildings.some(b => b.id === 'tent'))
        tips.push('⛺ Build a Tent — colonists need shelter');
    if (!G.buildings.some(b => b.id === 'campfire' || b.id === 'kitchen'))
        tips.push('🔥 Build a Campfire to cook food');
    if (G.res.food < 20)
        tips.push('🌾 LOW FOOD — build Field or send Hunter');
    if (G.res.water < 20)
        tips.push('💧 LOW WATER — send Porter to water source');
    if (alive.some(c => c.sick))
        tips.push('🤒 Someone is ill — assign a Medic');
    if (alive.some(c => c.hunger > 70))
        tips.push('🍽 Colonists are hungry!');
    if (alive.some(c => c.thirst > 70))
        tips.push('💧 Colonists are thirsty!');
    body.innerHTML = (tips.length ? tips : ['✅ Colony stable']).map(t => `<div class="adv-tip">${t}</div>`).join('');
}
// ── MODAL ─────────────────────────────────────────────────────────────────────
function showModal(title, text, btns, _danger = false) {
    G.paused = true;
    updPauseBtn();
...
function loop() {
    ...
    if (G.paused)
        return;
    const now = Date.now();
    if (now - lastTick < TICK_MS / G.speed)
        return;
    lastTick = now;
    tick();
}
function tick() {
    ...
    if (G.minute % 5 === 0)
        G.colonists.filter(c => !c.dead).forEach(doWork);
    if (G.minute % 3 === 0)
        tickConstruction();
    if (G.minute % 10 === 0)
        tickPiles();
    ...
    if (G.enemies?.length) {
        ...
        G.colonists.filter(co => !co.dead && co.role !== 'GUARD').forEach(co => {
            ...
            if (nearEnemy && co.action !== 'FLEEING') {
                co.targetCol = G.hqCol;
                co.targetRow = G.hqRow;
                co.action = 'FLEEING';
            }
            else if (!nearEnemy && co.action === 'FLEEING')
                co.action = 'IDLE';
        });
    }
    checkFirstRaid();
...
function hourTick() {
    const alive = G.colonists.filter(c => !c.dead);
    alive.forEach(c => {
        if (c.hunger === 60 && !c._warnedHunger) {
            c._warnedHunger = true;
            addLog(c.name + ' is getting hungry', 'warn');
        }
        if (c.thirst === 60 && !c._warnedThirst) {
            c._warnedThirst = true;
            addLog(c.name + ' is getting thirsty', 'warn');
        }
        if (c.hunger < 60)
            c._warnedHunger = false;
        if (c.thirst < 60)
            c._warnedThirst = false;
        if (c.hunger >= 80) {
            const drain = Math.floor((c.hunger - 79) / 10) + 1;
            c.hp = Math.max(0, c.hp - drain);
            if (c.hunger >= 90 && !c._warnedStarving) {
                c._warnedStarving = true;
                addLog('⚠ ' + c.name + ' is STARVING! HP draining!', 'danger');
            }
        }
        else
            c._warnedStarving = false;
        if (c.thirst >= 80) {
            const drain = Math.floor((c.thirst - 79) / 8) + 1;
            c.hp = Math.max(0, c.hp - drain);
            if (c.thirst >= 90 && !c._warnedDehydrated) {
                c._warnedDehydrated = true;
                addLog('⚠ ' + c.name + ' DEHYDRATED! HP draining!', 'danger');
            }
        }
        else
            c._warnedDehydrated = false;
        if (c.hp <= 0 && !c.dead) {
            c.dead = true;
            const cause = c.thirst >= 90 ? 'dehydration' : c.hunger >= 90 ? 'starvation' : c.sick ? 'illness' : 'wounds';
            addLog(c.name + ' died of ' + cause + '.', 'danger');
            onDeath(c);
        }
        if (c.hunger >= 100 && !c.dead) {
            c.dead = true;
            addLog(c.name + ' starved.', 'danger');
            onDeath(c);
        }
        if (c.thirst >= 100 && !c.dead) {
            c.dead = true;
            addLog(c.name + ' died of thirst.', 'danger');
            onDeath(c);
        }
        const tr = TRAITS.find(t => t.id === c.trait);
        if (!c.sick && Math.random() < 0.0007 * tr.sick) {
            c.sick = true;
            c.sickTimer = rnd(12, 48);
            addLog(c.name + ' fell ill - needs a medic!', 'warn');
        }
        if (c.sick) {
            c.sickTimer--;
            c.hp = Math.max(1, c.hp - rnd(1, 3));
            if (c.sickTimer <= 0) {
                c.sick = false;
                addLog(c.name + ' recovered', 'good');
            }
            if (c.hp <= 20 && !c.deathWarned) {
                c.deathWarned = true;
                addLog('⚠ ' + c.name + ' is critically ill!', 'danger');
            }
        }
        else {
            c.deathWarned = false;
            if (c.hp < c.maxHp && c.hunger < 50 && c.thirst < 50)
        ...
        if (!hasShelter && (G.hour < 6 || G.hour >= 20))
            md -= 1.5;
        const topPref = PROF.reduce((a, b) => (c.prefs[a] || 1) >= (c.prefs[b] || 1) ? a : b);
        if (c.role !== topPref && (c.skill[c.role] || 0) < 50)
            md -= 0.5;
        if (c.hunger < 30 && c.thirst < 30)
            md += 0.3;
        if (c.role === topPref && !c.sleeping)
            md += 0.4;
        if (hasShelter)
            md += 0.2;
            ...
            showModal('RAID!', 'A band of raiders is attacking your camp!\nYour colonists will defend automatically.\n\nKnife-armed colonists fight. Others flee to HQ.', [{
                    label: 'DEFEND', cls: 'danger', fn: () => {
                        ...
                        if (fighters.length < 3 && G.colonists.filter(c => !c.dead).length > 1) {
                            ...
                            if (victim) {
                                victim.hp = Math.max(0, victim.hp - rnd(20, 50));
...
function dayTick() {
    ...
    if (G.seasonDay >= 10) {
        ...
        addLog('Season: ' + SEASONS[G.season], 'good');
    }
    G.tiles.filter(t => t.bldg?.field && t.bldg.phase === 'growing').forEach(t => {
        t.bldg.growth = Math.min(100, (t.bldg.growth || 0) + rnd(8, 15));
        if (t.bldg.growth >= 100) {
            t.bldg.phase = 'harvest';
            addLog(t.bldg.crop + ' ready to harvest!', 'good');
            refreshTileEl(t);
        }
        else
            refreshTileEl(t);
...
function onDeath(c) {
    ...
    showModal('SURVIVOR LOST', c.name + ' has died.\n\nBuild a Grave (OTHER tab) — an unburied body\ndamages the morale of survivors nearby.', [{ label: 'UNDERSTOOD', cls: 'ok', fn: () => { G.paused = false; updPauseBtn(); } }]);
    renderSidebar();
}
function randEvent() {
    const evs = [
...
function openCraftShop(shopId) {
    ...
    showModal(name, header + 'Select item to craft.', available.map(r => {
        const canAfford = Object.entries(r.cost).every(([k, v]) => (G.res[k] || 0) >= v);
        ...
        return {
            ...
            fn: () => {
                if (!canAfford) {
                    addLog('Not enough for ' + r.name, 'warn');
                    G.paused = false;
                    updPauseBtn();
                    return;
                }
                ...
                if (r.weapon) {
                    ...
                    if (t) {
                        t.weapon = { type: r.toolType, dur: r.dur };
                        addLog(r.ico + ' ' + r.name + ' → ' + t.name, 'good');
                    }
                    else
                        addLog(r.ico + ' ' + r.name + ' stored', 'good');
                }
                else {
                    const t = G.colonists.find(co => !co.dead && co.role === r.role && (!co.tool || co.tool.dur < 50)) || G.colonists.find(co => !co.dead && co.role === r.role);
                    if (t) {
                        t.tool = { type: r.toolType, dur: r.dur };
                        addLog(r.ico + ' ' + r.name + ' → ' + t.name, 'good');
                    }
                    else
                        addLog(r.ico + ' ' + r.name + ' stored (no ' + r.role + ')', 'good');
                }
                renderResources();
                G.paused = false;
                updPauseBtn();
...
function tickCombat() {
    ...
    alive.filter(c => c.role === 'GUARD').forEach(guard => {
        ...
        if (nearEnemy) {
            guard.targetCol = nearEnemy.col;
            guard.targetRow = nearEnemy.row;
            guard.action = 'FIGHTING';
            ...
            if (guard.attackTimer >= 18) {
                ...
                nearEnemy.hp -= dmg;
                punchAnim('sp-' + guard.id);
                punchAnim('enemy-' + nearEnemy.id);
                addLog('⚔ ' + guard.name + ' hit raider for ' + dmg, nearEnemy.hp <= 10 ? 'good' : 'normal');
                if (nearEnemy.hp <= 0) {
                    addLog('Raider defeated!', 'good');
                    removeEnemy(nearEnemy);
                }
            }
        }
        else {
            ...
            if (closest) {
                guard.targetCol = closest.col;
                guard.targetRow = closest.row;
                guard.action = 'PURSUING';
            }
        }
    });
    alive.filter(c => c.role === 'MEDIC').forEach(medic => {
        ...
        if (wounded) {
            medic.targetCol = wounded.col;
            medic.targetRow = wounded.row;
            medic.action = 'HEALING ' + wounded.name;
            if (Math.hypot(medic.col - wounded.col, medic.row - wounded.row) < 1.0 && G.res.meds > 0) {
                G.res.meds--;
                wounded.hp = Math.min(wounded.maxHp, wounded.hp + rnd(15, 25));
...
function startGame() {
    ...
    G.colonists.forEach(c => {
        const needed = PROF_TOOL[c.role];
        if (needed)
            c.tool = { type: 'Stone ' + needed, dur: 80 };
        else
            c.tool = { type: '—', dur: 100 };
