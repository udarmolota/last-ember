...
function punchAnim(spriteId) {
  ...
  setTimeout(() => {
    if (sp) sp.style.transform = 'scale(1)';
  }, 150);
}
  ...
  G.colonists
    ...
    .forEach((c) => {
      const nearEnemy = G.enemies.some(
        (e) =>
          Math.sqrt(Math.pow(c.col - e.col, 2) + Math.pow(c.row - e.row, 2)) < 6
      );
      if (nearEnemy) {
        c.targetCol = G.hqCol;
        c.targetRow = G.hqRow;
...
function tickCombat() {
  ...
  G.enemies.forEach((enemy) => {
    ...
    const target = guards.length
      ? guards.reduce((a, b) =>
          Math.hypot(a.col - enemy.col, a.row - enemy.row) <
          Math.hypot(b.col - enemy.col, b.row - enemy.row)
            ? a
            : b
        )
    ...
    if (dist > 0.9) {
      ...
      if (enemy.attackTimer >= 20) {
        ...
        if ('hp' in target) {
          ...
          target.combatSkill = Math.min(
            100,
            (target.combatSkill || 0) + rnd(1, 3)
          );
        }
      }
    }
  ...
  alive
    .filter((c) => c.role === 'GUARD')
    .forEach((guard) => {
      const nearEnemy = G.enemies.find(
        (e) => !e.dead && Math.hypot(guard.col - e.col, guard.row - e.row) < 1.5
      );
      if (nearEnemy) {
        ...
        if (guard.attackTimer >= 18) {
          ...
          const dmg = calcDamage(
            {
              combatSkill: guard.combatSkill || 0,
              damage: 10 + (guard.weapon ? 5 : 0),
            },
            nearEnemy
          );
          nearEnemy.hp -= dmg;
          ...
          addLog(
            '⚔ ' + guard.name + ' hit raider for ' + dmg,
            nearEnemy.hp <= 10 ? 'good' : 'normal'
          );
          if (nearEnemy.hp <= 0) {
  ...
  alive
    .filter((c) => c.role === 'MEDIC')
    .forEach((medic) => {
      const wounded = alive.find(
        (x) => x.hp < x.maxHp * 0.7 && !x.dead && x.id !== medic.id
      );
      if (wounded) {
        ...
        if (
          Math.hypot(medic.col - wounded.col, medic.row - wounded.row) < 1.0 &&
          G.res.meds > 0
        ) {
          G.res.meds--;
